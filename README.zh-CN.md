...
![英文学习统计页：打卡日历、连续学习、趋势图与分数分布](docs/images/statistics.png)

统计页完全从本机词库和造句历史实时计算：六周打卡热力日历、当前与最长连胜、14 天练习/平均分/新增词趋势、分数分布、掌握阶段、主动回忆覆盖率、稳定掌握率和需要继续巩固的词。
...
- 提交后生成 2–3 条彼此独立的“搭配模式 + 直接含义”，禁止用斜杠合并成一大段；
- 最近 10 张用法卡在首页横向轮播；
- 错词候选规范化、去重，独立显示词性和释义，并由用户确认是否收录；
- AI 多维证据自动映射内部记忆等级；
- 低置信度自动二次核验，冲突时不污染 FSRS 长期状态；
- 打卡日历、连续学习火苗、练习与分数趋势、掌握阶段和待巩固词统计；
- 简体中文与 English 界面；
- 目标语言和 AI 解释语言分别配置；
...
默认模型是 `stealth/ox-alpha`，可在设置中改为 `deepseek/deepseek-v4-flash-0731` 等 OpenRouter 模型。DeepSeek V4 Flash 会自动使用低推理强度，通过一次结构化请求同时完成批改和目标词证据，仅在证据不确定时二次核验，并支持空内容重试和更大的最终输出预算。密钥依次从进程、当前用户和系统环境变量读取，不会写入项目或数据文件。
