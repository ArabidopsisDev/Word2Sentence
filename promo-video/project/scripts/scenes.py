...
def open_source_intro(ctx: SceneContext, visual: dict) -> None:
    ...
    code_alpha = alpha * _ease(ctx, 0.9, 0.6)
    _code_panel(ctx, (118, 272, 955, 365), list(visual["code"]), alpha=code_alpha, trace_ids=["license-code"], token_kinds=[{"token": "AGPL-3.0-only", "kind": "string"}])
...
def word_gap(ctx: SceneContext, visual: dict) -> None:
    ...
    _paste_image(ctx, str(visual["app_image"]), (260, 118, 1660, 820), alpha=alpha * full, radius=18, trace_id="library-window")
    ...
    _tag(ctx, (1102, 630), str(visual["screen_text"][0]), alpha=alpha * _ease(ctx, 7.0, 0.45), active=False, trace_id="word-summary")
    _tag(ctx, (1400, 630), str(visual["screen_text"][1]), alpha=alpha * _ease(ctx, 9.5, 0.45), active=True, trace_id="gap-summary")
    _code_panel(ctx, (1130, 112, 1785, 170), list(visual["code"]), alpha=0.9 * alpha, trace_ids=["license-code"], token_kinds=[{"token": "AGPL-3.0-only", "kind": "string"}])
...
def sentence_flow(ctx: SceneContext, visual: dict) -> None:
    ...
    source_alpha = alpha * (1.0 - _ease(ctx, 5.0, 0.6))
    ...
    overlay_progress = _ease(ctx, 5.0, 0.7)
    practice_alpha = alpha * _ease(ctx, 2.7, 0.6) * (1.0 - 0.43 * overlay_progress)
    x1 = round(_lerp(980, 235, _ease(ctx, 5.0, 0.7)))
    x2 = round(_lerp(1810, 1690, _ease(ctx, 5.0, 0.7)))
    ...
    draw_text(ctx, (372, 580), typed, size=32, kind="sans", fill=INK, alpha=input_alpha, trace_id="sentence-input")
    ...
    _code_panel(ctx, (105, 106, 650, 206), list(visual["code"]), alpha=alpha * _ease(ctx, 9.4, 0.45), trace_ids=["challenge-method", "evaluate-method"], explicit_kinds=["method", "method"])
...
def feedback_artifacts(ctx: SceneContext, visual: dict) -> None:
    ...
    base_alpha = alpha * (1.0 - 0.72 * dialog_in)
    _paste_image(ctx, str(visual["app_image"]), (180, 128, 1740, 815), crop=(320, 250, 1430, 990), alpha=base_alpha, radius=18, trace_id="feedback-window")
    segment_alpha = base_alpha * _ease(ctx, 1.5, 0.45)
    _draw_segments(ctx, list(visual["segments"]), (315, 322), segment_alpha, progress(ctx.scene_t, 1.6, 2.5))
    version_alpha = base_alpha * _ease(ctx, 5.2, 0.55)
    ...
    usage_alpha = base_alpha * _ease(ctx, 9.0, 0.55)
    ...
    pattern_alpha = base_alpha * _ease(ctx, 10.5, 0.45)
    ...
    draw_line(ctx, (1160, 770), (1160 + 625 * usage_line, 770), color=GREEN, width=4, alpha=base_alpha * usage_line, trace_id="usage-card")
    code_alpha = base_alpha * _ease(ctx, 6.0, 0.45)
    _code_panel(ctx, (1180, 112, 1785, 274), list(visual["code"]), alpha=code_alpha, trace_ids=["corrected-code", "better-code", "usage-code"])
...
def automatic_scheduler(ctx: SceneContext, visual: dict) -> None:
    ...
    settings_alpha = alpha * (0.30 + 0.24 * _ease(ctx, 15.0, 0.55))
    _paste_image(ctx, str(visual["app_image"]), (90, 205, 760, 770), crop=(270, 560, 1060, 960), alpha=settings_alpha, radius=18, trace_id="settings-evidence")
    draw_text(ctx, (400, 365), str(visual["word"]), size=54, kind="sans_bold", fill=GREEN, alpha=alpha, anchor="mm")
    evidence = list(visual["evidence"])
    positions = [(870, 245), (1260, 245), (870, 365), (1260, 365)]
    for index, (item, position) in enumerate(zip(evidence, positions)):
        event_alpha = alpha * _ease(ctx, 2.0 + index * 0.7, 0.45)
        _tag(ctx, position, str(item), alpha=event_alpha, active=index < 2, trace_id="evidence-points" if index < 2 else "behavior-points")
    merge_alpha = alpha * _ease(ctx, 6.0, 0.5)
    for index, position in enumerate(positions):
        arrow_alpha = alpha * _ease(ctx, 6.0 + index * 0.62, 0.48)
        draw_arrow(ctx, (position[0] + 130, position[1] + 54), (1160, 548), color=GREEN, width=3, alpha=0.35 * arrow_alpha, trace_id="evidence-merge")
    _panel(ctx, (960, 520, 1360, 605), fill=GREEN_SOFT, outline=GREEN, radius=16, alpha=merge_alpha, trace_id="evidence-merge")
    draw_text(ctx, (1160, 563), str(visual["screen_text"][0]), size=34, kind="sans_bold", fill=GREEN, alpha=merge_alpha, anchor="mm")
    timeline_alpha = alpha * _ease(ctx, 10.6, 0.5)
    draw_line(ctx, (790, 715), (1650, 715), color=BORDER, width=5, alpha=timeline_alpha, trace_id="fsrs-timeline")
    draw_text(ctx, (810, 755), str(visual["timeline"][0]), size=23, kind="sans", fill=MUTED, alpha=timeline_alpha, anchor="mm")
    draw_text(ctx, (1630, 755), str(visual["timeline"][1]), size=23, kind="sans_bold", fill=GREEN, alpha=timeline_alpha, anchor="mm")
    marker_progress = smooth(progress(ctx.scene_t, 11.2, 2.2))
    marker_x = _lerp(810, 1630, marker_progress)
    ...
    ctx.draw.ellipse((marker_x - 13, 702, marker_x + 13, 728), fill=with_alpha(GREEN, timeline_alpha))
    _code_panel(ctx, (760, 610, 1735, 682), list(visual["code"]), alpha=alpha * _ease(ctx, 11.7, 0.45), trace_ids=["scheduler-code"])
    result_alpha = alpha * _ease(ctx, 16.5, 0.45) * (1.0 - _ease(ctx, 19.0, 0.35))
    _tag(ctx, (1470, 760), str(visual["timeline"][1]), alpha=result_alpha, active=True, trace_id="fsrs-result")
    conflict = list(visual["conflict"])
    conflict_alpha = alpha * _ease(ctx, 19.0, 0.5)
    _panel(ctx, (120, 640, 675, 802), fill=RED_SOFT, outline=RED_DARK, radius=16, alpha=conflict_alpha, trace_id="conflict-branch")
    draw_text(ctx, (165, 685), str(conflict[0]), size=25, kind="sans_bold", fill=RED_DARK, alpha=conflict_alpha)
    draw_text(ctx, (165, 732), str(conflict[1]), size=25, kind="sans_bold", fill=INK, alpha=conflict_alpha * _ease(ctx, 20.4, 0.4), trace_id="retest-note")
    draw_text(ctx, (165, 770), str(conflict[2]), size=21, kind="sans", fill=MUTED, alpha=conflict_alpha * _ease(ctx, 22.0, 0.4), trace_id="retest-note")
    note_alpha = alpha * _ease(ctx, 24.0, 0.5)
    _tag(ctx, (1180, 760), str(visual["screen_text"][1]), alpha=note_alpha, active=True, trace_id="no-rating-note")
    note_line = smooth(progress(ctx.scene_t, 26.0, 0.5))
    draw_line(ctx, (1180, 820), (1180 + 430 * note_line, 820), color=GREEN, width=4, alpha=alpha * note_line, trace_id="no-rating-note")


def data_boundary(ctx: SceneContext, visual: dict) -> None:
    alpha = _ease(ctx)
    _chapter(ctx, alpha)
    _paste_image(ctx, str(visual["app_image"]), (80, 135, 1840, 815), crop=(240, 110, 1120, 990), alpha=0.20 * alpha, radius=18, trace_id="settings-window")
    draw_text(ctx, (960, 145), str(visual["headline"]), size=40, kind="sans_bold", fill=INK, alpha=alpha, anchor="mm")
    local_alpha = alpha * _ease(ctx, 0.8, 0.45)
    api_alpha = alpha * _ease(ctx, 3.5, 0.45)
    _paste_image(ctx, str(visual["app_image"]), (150, 250, 830, 460), crop=(270, 610, 1060, 940), alpha=0.58 * local_alpha, radius=18, trace_id="local-route")
    _panel(ctx, (185, 270, 795, 350), fill=CREAM, outline=BORDER, radius=14, alpha=local_alpha, trace_id="local-route")
    draw_text(ctx, (490, 315), str(visual["routes"][0]), size=30, kind="sans_bold", fill=GREEN, alpha=local_alpha, anchor="mm")
    draw_arrow(ctx, (490, 350), (490, 415), color=GREEN, width=4, alpha=local_alpha, trace_id="local-route")
    _paste_image(ctx, str(visual["app_image"]), (1090, 250, 1770, 460), crop=(270, 390, 1060, 580), alpha=0.58 * api_alpha, radius=18, trace_id="api-route")
    _panel(ctx, (1125, 270, 1735, 350), fill=CREAM, outline=BORDER, radius=14, alpha=api_alpha, trace_id="api-route")
    draw_text(ctx, (1430, 315), str(visual["routes"][1]), size=28, kind="sans_bold", fill=BLUE_DARK, alpha=api_alpha, anchor="mm")
    draw_arrow(ctx, (1430, 350), (1430, 415), color=BLUE_DARK, width=4, alpha=api_alpha, trace_id="api-route")
    provider_alpha = alpha * _ease(ctx, 5.7, 0.45)
    _tag(ctx, (1280, 478), str(visual["provider"]), alpha=provider_alpha, active=False, trace_id="provider-route")
    draw_arrow(ctx, (1430, 448), (1430, 478), color=BLUE_DARK, width=3, alpha=provider_alpha, trace_id="provider-route")
    recent_alpha = alpha * _ease(ctx, 8.4, 0.45)
    _tag(ctx, (1120, 570), str(visual["recent_context"]), alpha=recent_alpha, active=False, trace_id="recent-context")
    draw_arrow(ctx, (1260, 595), (1370, 520), color=BLUE_DARK, width=3, alpha=0.7 * recent_alpha, trace_id="recent-context")
    _code_panel(ctx, (170, 485, 1040, 605), list(visual["code"]), alpha=alpha * _ease(ctx, 11.0, 0.4), trace_ids=["data-path-code", "language-code"])
    language_alpha = alpha * _ease(ctx, 13.2, 0.5)
    for index, item in enumerate(list(visual["languages"])):
        _tag(ctx, (470 + index * 350, 690), str(item), alpha=language_alpha * _ease(ctx, 13.2 + index * 0.35, 0.35), active=index == 1, trace_id="language-settings")
    draw_text(ctx, (490, 645), str(visual["screen_text"][0]), size=30, kind="sans_bold", fill=GREEN, alpha=local_alpha, anchor="mm")
    language_line = smooth(progress(ctx.scene_t, 16.0, 0.5))
    draw_line(ctx, (470, 760), (1330, 760), color=GREEN, width=4, alpha=alpha * language_line, trace_id="language-settings")
...
def summary(ctx: SceneContext, visual: dict) -> None:
    ...
    for index, item in enumerate(items):
        item_alpha = alpha * _ease(ctx, 0.8 + index * 0.45, 0.4)
